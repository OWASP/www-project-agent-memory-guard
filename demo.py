...
def format_blocked_message(exc: PolicyViolation, content: str) -> str:
    display = content[:60] + "..." if len(content) > 60 else content
    rule = exc.rule or "policy"
    return f"  🛡️  BLOCKED [{rule}]: {display}"
...
def main() -> None:
    ...
    for key, content in poisoned_memories:
        try:
            ...
            blocked_count += 1
            print(format_blocked_message(exc, content))
    print()
